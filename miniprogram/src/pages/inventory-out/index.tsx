...
export default function InventoryOut() {
  ...
  const handleSubmit = async () => {
    ...
    if (!parsedQuantity || parsedQuantity <= 0) {
      Taro.showToast({ title: "出库数量必须大于0", icon: "none" });
      return;
    }
    ...
    try {
      ...
      Taro.showToast({
        title: `出库成功，当前库存：${result.after_quantity} ${result.unit || stockInfo?.unit || ""}`,
        icon: "success",
      });
      ...
      if (result.low_stock) {
        setTimeout(() => {
          Taro.showModal({
            title: "低库存提醒",
            content: `${result.reagent_name || stockInfo?.name_cn || "当前试剂"}库存已低于或等于预警阈值，请及时补充。`,
            showCancel: false,
          });
        }, 900);
      }
    } catch (error) {
      // 库存不足等后端错误会通过 request 封装原样展示给用户。
  ...
  return (
    <View className="page inventory-out-page">
      <Text className="page-title">试剂出库</Text>
      <Text className="page-subtitle">从试剂详情页带入试剂后，记录实验领用、消耗等出库操作。</Text>

      {!reagentId ? (
        <View className="panel empty-panel">
          <Text className="panel-title">请先选择试剂</Text>
          <Text className="muted-text">出库操作需要先确定具体试剂，避免手动输入 ID 造成误操作。</Text>
        ...
        <>
          <View className="panel stock-card">
            ...
            {stockInfo?.low_stock && (
              <View className="low-stock-notice">
                <Text className="low-stock-text">当前库存已低于或等于预警阈值，请谨慎出库。</Text>
              </View>
            )}
          ...
          <View className="panel">
            <Text className="form-label">出库数量</Text>
            <Input
            ...
            <Textarea
              ...
              placeholder="可填写领用人、课题组或其他说明"
              value={remark}
              onInput={(event) => setRemark(event.detail.value)}
            />
          </View>

          <Button className="danger-button" loading={submitting} onClick={handleSubmit}>
            提交出库
          </Button>
